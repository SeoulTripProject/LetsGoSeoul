		...
		try
		{
			...
			for(String cls:clsList)
			{
				...
				// Controller
				  ...
				  if(clsName.isAnnotationPresent(Controller.class)==false) // Controller 어노테이션이
				      continue;// 제외
				 				// 메모리 할당(클래스)
				...
				// 클래스(모델)안에 선언된 모든 클래스를 가지고 온다 
				// RequestMapping에 해당된 메소드를 찾아서 수행 
				for(Method m:methods)
				{
					// 어노테이션을 찾아서 => 밑에 있는 메소드 호출 
					RequestMapping rm=m.getAnnotation(RequestMapping.class);
					// @RequestMapping("movie/list.do") => rm.value()="movie/list.do"
					if(rm.value().equals(cmd))
					{
						...
						if(jsp.startsWith("redirect"))
						{
							...
							// request가 필요가 없는 경우 (jsp로 값을 보내지 않고 초기화)
						}
						else
						{
							// request에 담긴 데이터를 출력할 목적 => request를 받는다 forward
			...
			// 3. 메소드가 호출 => request/session을 JSP전송 => forward => 520page
		}catch(Exception ex) {ex.printStackTrace();}
	}
